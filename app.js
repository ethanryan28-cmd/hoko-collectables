...
// ─── State ────────────────────────────────────────────────────
// BUG FIX: Wrap localStorage reads in try/catch so malformed data never breaks the app
function safeLoadStorage(key, fallback) {
    try {
          const raw = localStorage.getItem(key);
          return raw ? JSON.parse(raw) : fallback;
    } catch (e) {
          console.warn('Hoko: could not parse localStorage key "' + key + '", resetting.', e);
          localStorage.removeItem(key);
          return fallback;
    }
}

let cart     = safeLoadStorage('hokoCart', []);
let wishlist = safeLoadStorage('hokoWishlist', []);
...
// BUG FIX: Escape any dynamic strings before injecting into innerHTML
...
function productCardHTML(p) {
  ...
  return `
      <div class="product-card" data-id="${p.id}" data-category="${safeCategory}">
            ${p.badge ? `<span class="product-badge badge-${safeBadge}">${safeBadge}</span>` : ''}
                  <button class="product-wishlist ${inWishlist ? 'wishlisted' : ''}" onclick="toggleWishlist(${p.id})" aria-label="Toggle wishlist" title="${inWishlist ? 'Remove from wishlist' : 'Add to wishlist'}">
                          <svg width="16" height="16" viewBox="0 0 24 24" fill="${inWishlist ? 'currentColor' : 'none'}" stroke="currentColor" stroke-width="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
                                      ...
                                      <div class="product-img">${safeEmoji}</div>
...
function renderCartItems() {
  ...
  container.innerHTML = cart.map(item => {
        ...
        return `
              <div class="cart-item">
                      <div class="cart-item-img">${escapeHTML(p.emoji)}</div>
...
function toggleWishlist(id) {
  ...
  // BUG FIX: Use $$ (querySelectorAll) instead of $ (querySelector) so all
  // matching cards on the page get updated, not just the first one found.
  $$(`[data-id="${id}"] .product-wishlist`).forEach(el => {
...
// ─── Newsletter Form ──────────────────────────────────────────
// BUG FIX: Use template literals (backtick strings) so apostrophes in
// the success message don't cause syntax errors.
$('#newsletterForm')?.addEventListener('submit', e => {
    e.preventDefault();
    const name  = $('#nlName').value.trim();
    const email = $('#nlEmail').value.trim();
    if (!name || !email) return;
    showToast(`🎉 Thanks ${escapeHTML(name)}! Check ${escapeHTML(email)} for your 10% off code.`);
    e.target.reset();
});

// ─── Contact Form ─────────────────────────────────────────────
// BUG FIX: Use template literal here too — the original single-quoted string
// contained an unescaped apostrophe in "We'll" which was a syntax error.
...
function openModal(id) {
  ...
  if (imgEl)     imgEl.textContent     = p.emoji;
    if (catEl)     catEl.textContent     = p.category.charAt(0).toUpperCase() + p.category.slice(1);
    if (titleEl)   titleEl.textContent   = p.name;
    if (starsEl)   starsEl.textContent   = '★'.repeat(Math.floor(p.rating)) + (p.rating % 1 >= 0.5 ? '½' : '');
    if (reviewsEl) reviewsEl.textContent = `(${p.reviews} reviews)`;
    if (descEl)    descEl.textContent    = PRODUCT_DESCS[p.category] || '';

  if (badgeEl) {
        badgeEl.textContent = p.badge || '';
        badgeEl.className   = 'product-badge' + (p.badge ? ` badge-${p.badge}` : '');
